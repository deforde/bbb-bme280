...
bool i2c_read_reg(int i2c_dev, uint8_t dev_addr, uint8_t reg_addr,
                  uint8_t *reg_val) {
  struct i2c_msg messages[] = {{dev_addr, 0, 1, &reg_addr},
                               {dev_addr, I2C_M_RD, 1, reg_val}};
  const size_t n_messages = sizeof(messages) / sizeof(*messages);
  const struct i2c_rdwr_ioctl_data ioctl_data = {.msgs = messages,
                                                 .nmsgs = n_messages};
...
bool i2c_write_reg(int i2c_dev, uint8_t dev_addr, uint8_t reg_addr,
                   uint8_t reg_val) {
