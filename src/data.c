...
bool read_data_from_file(const char* file_path, data_t** data, size_t* n_data_points)
{
    ...
    {
        printf("Binary content of: \"%s\", is not an integer multiple of sizeof(data_t)\n", file_path);
    ...
    else
    {
        ...
        const size_t n_data_points_read = fread(buf, sizeof(data_t), n_data_points_expected, file);
        if (n_data_points_read != n_data_points_expected)
        {
            fprintf(stderr, "Failed to read %u data points from the file: \"%s\" (actual num points read: %u)\n", n_data_points_expected, file_path, n_data_points_read);
            free(buf);
            ret_val = false;
        }
        else
...
bool data_vec_create(data_vec_t* data_vec)
{
    ...
    data_vec->data = (data_t*)malloc(DEFAULT_DATA_VEC_CAPACITY * sizeof(data_t));
    if (data_vec->data == NULL)
    {
        fprintf(stderr, "Failed to allocate initial data_vec_t memory (size: %u bytes)\n", DEFAULT_DATA_VEC_CAPACITY * sizeof(data_t));
        return false;
    }
...
bool data_vec_push(data_vec_t* data_vec, data_t data)
{
    ...
    {
        const size_t new_capacity = DEFAULT_DATA_VEC_GROWTH_FACTOR * data_vec->capacity;
        data_t* new_data_buf = (data_t*)malloc(new_capacity * sizeof(data_t));

        if (new_data_buf == NULL)
        {
            fprintf(stderr, "Data vec at capcity but failed to allocate additional memory, could not push new data to vec (alloc size: %u bytes)\n", new_capacity);
            return false;
        }

        memcpy(new_data_buf, data_vec->data, data_vec->capacity * sizeof(data_t));
        free(data_vec->data);
