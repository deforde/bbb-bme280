...
static bool read_val_20u(int i2c_dev, const uint8_t *const addrs,
                         size_t n_addrs, int32_t *val) {
  assert(n_addrs == 3);
...
static bool read_val_16u(int i2c_dev, const uint8_t *const addrs,
                         size_t n_addrs, int32_t *val) {
  assert(n_addrs == 2);
...
static bool read_val_16s(int i2c_dev, const uint8_t *const addrs,
                         size_t n_addrs, int32_t *val) {
...
bool bme_get_temp(int i2c_dev, float *t_degC, int32_t *t_fine) {
  ...
  if (!read_val_20u(i2c_dev, adc_T_addrs,
                    sizeof(adc_T_addrs) / sizeof(*adc_T_addrs), &adc_T)) {
    return false;
  }

  int32_t dig_T1 = 0;
  const uint8_t dig_T1_addrs[] = {0x89, 0x88};
  if (!read_val_16u(i2c_dev, dig_T1_addrs,
                    sizeof(dig_T1_addrs) / sizeof(*dig_T1_addrs), &dig_T1)) {
    return false;
  }

  int32_t dig_T2 = 0;
  const uint8_t dig_T2_addrs[] = {0x8B, 0x8A};
  if (!read_val_16s(i2c_dev, dig_T2_addrs,
                    sizeof(dig_T2_addrs) / sizeof(*dig_T2_addrs), &dig_T2)) {
    return false;
  }

  int32_t dig_T3 = 0;
  const uint8_t dig_T3_addrs[] = {0x8D, 0x8C};
  if (!read_val_16s(i2c_dev, dig_T3_addrs,
                    sizeof(dig_T3_addrs) / sizeof(*dig_T3_addrs), &dig_T3)) {
    return false;
  }
...
bool bme_get_pressure(int i2c_dev, int32_t t_fine, float *p_kPa) {
  ...
  if (!read_val_20u(i2c_dev, adc_P_addrs,
                    sizeof(adc_P_addrs) / sizeof(*adc_P_addrs), &adc_P)) {
    return false;
  }

  int64_t dig_P1 = 0;
  const uint8_t dig_P1_addrs[] = {0x8F, 0x8E};
  if (!read_val_16u(i2c_dev, dig_P1_addrs,
  ...
  int32_t dig_P[8] = {0};
  const uint8_t dig_P_addrs[8][2] = {{0x91, 0x90}, {0x93, 0x92}, {0x95, 0x94},
                                     {0x97, 0x96}, {0x99, 0x98}, {0x9B, 0x9A},
                                     {0x9D, 0x9C}, {0x9F, 0x9E}};
  for (size_t i = 0; i < sizeof(dig_P_addrs) / sizeof(*dig_P_addrs); ++i) {
    if (!read_val_16s(i2c_dev, dig_P_addrs[i],
...
bool bme_get_humidity(int i2c_dev, int32_t t_fine, float *humidity_pcnt) {
  ...
  if (!read_val_16u(i2c_dev, adc_H_addrs,
                    sizeof(adc_H_addrs) / sizeof(*adc_H_addrs), &adc_H)) {
    return false;
  }
  ...
  if (!read_val_16s(i2c_dev, dig_H2_addrs,
                    sizeof(dig_H2_addrs) / sizeof(*dig_H2_addrs), &dig_H2)) {
    return false;
  }
  ...
  v_x1_u32r =
      v_x1_u32r -
      (((((v_x1_u32r >> 15) * (v_x1_u32r >> 15)) >> 7) * INT32_C(dig_H1)) >> 4);
