...
char *plot_generate_html(const data_t *const data, size_t n_data_points) {
  ...
  for (size_t i = 0; i < n_data_points; ++i) {
    ...
    temp_data_str_idx +=
        snprintf(&temp_data_str[temp_data_str_idx],
                 temp_data_str_len - temp_data_str_idx, "%.2f", data[i].t_degC);
  ...
  const size_t total_html_str_len =
      strlen(html_0) + strlen(html_1) + strlen(html_2) + strlen(html_3) +
      strlen(html_4) + strlen(time_data_str) + strlen(temp_data_str) +
      strlen(press_data_str) + strlen(humid_data_str) + 1;
